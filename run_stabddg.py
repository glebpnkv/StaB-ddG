...
def run(model, dataset, ensemble=20, batch_size=10000):
    ...
    for sample in tqdm(dataset):
        ...
        binding_ddG_pred *= (
            -1
        )  # Convert to convention of negative values for stabilizing mutations
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--pdb_path", type=str, default="examples/one_mutation/1AO7.pdb"
    )
    ...
    argparser.add_argument(
        "--noise_level", type=float, default=0.1, help="amount of backbone noise"
    )
    ...
    if not args.pdb_dir:
        ...
        pdb_dir = os.path.join(
            os.path.dirname(args.pdb_path), pdb_file_name + "_output"
        )
        ...
        extract_chains(
            complex_path,
            f"{pdb_dir}/{pdb_file_name}_{binder1_chains}.pdb",
            binder1_chains,
            f"{pdb_dir}/{pdb_file_name}_{binder2_chains}.pdb",
            binder2_chains,
        )
        ...
        df = pd.DataFrame([data])
        csv_path = os.path.join(pdb_dir, "input.csv")
        df.to_csv(csv_path, index=False)
        pdb_cache_path = os.path.join(
            pdb_dir, f"{pdb_file_name}_{binder1_chains}_{binder2_chains}_cache.pkl"
        )
        output_dir = pdb_dir
    else:
        output_dir = (
            args.output_dir
            if args.output_dir
            else os.path.join(os.path.dirname(args.csv_path), "output")
        )
        ...
        pdb_cache_path = (
            f"{pdb_dir}/{args.pdb_dict_cache_name}" if args.pdb_dict_cache_name else ""
        )
    ...
    dataset = SKEMPIDataset(
        pdb_dir=pdb_dir, csv_path=csv_path, pdb_dict_cache_path=pdb_cache_path
    )
    ...
    with torch.no_grad():
        for i in tqdm(range(args.trials)):
            pred_df = run(
                model, dataset, ensemble=args.mc_samples, batch_size=args.batch_size
            )
            if i == 0:
